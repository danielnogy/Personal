...
public class ApplicationUserManager(IUserStore<ApplicationUser> store,
                                    ...
                                    ITenantResolver tenantResolver) : UserManager<ApplicationUser>(store, optionsAccessor, passwordHasher, userValidators, passwordValidators, keyNormalizer, errors, services, logger)
{
    ...
    public async Task<string> SendActivationEmailAsync(ApplicationUser user)
    {
        ...
        var url = $"{tenantHostName}/{appClientOptions.ConfirmEmailUrl}";
    ...
    public async Task<string> SendActivationEmailAsync(ApplicationUser user, string newEmail)
    {
        ...
        if (Options.SignIn.RequireConfirmedAccount)
        {
            ...
            // Generate callback URL for the activation email.
            var url = $"{tenantHostName}/{appClientOptions.ConfirmEmailChangeUrl}";
    ...
    public async Task<string> SendResetPasswordAsync(ApplicationUser user)
    {
        ...
        var tenantMode = tenantResolver.TenantMode;

        // Determine the appropriate URL based on the tenant mode.
        var url = tenantMode switch
        {
            TenantMode.MultiTenant => $"{string.Format(clientAppOptions.MultiTenantHostName, appOptionsService.GetSubDomain())}/{clientAppOptions.ResetPasswordUrl}",
