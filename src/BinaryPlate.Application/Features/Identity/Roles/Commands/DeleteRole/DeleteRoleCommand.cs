...
public class DeleteRoleCommand : IRequest<Envelope<string>>
{
    ...
    public class DeleteRoleCommandHandler(ApplicationRoleManager roleManager) : IRequestHandler<DeleteRoleCommand, Envelope<string>>
    {
        ...
        public async Task<Envelope<string>> Handle(DeleteRoleCommand request, CancellationToken cancellationToken)
        {
            ...
            var role = await roleManager.Roles.Include(r => r.RoleClaims).FirstOrDefaultAsync(r => r.Id == request.Id, cancellationToken: cancellationToken);
            ...
            if (identityResult.Succeeded)
                await roleManager.RemoveExcludedPermissionsFromAllUsers(role);
