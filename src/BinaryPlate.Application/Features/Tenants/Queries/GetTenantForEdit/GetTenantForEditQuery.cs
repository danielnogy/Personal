...
public class GetTenantForEditQuery : IRequest<Envelope<GetTenantForEditResponse>>
{
    ...
    public class GetTenantForEditQueryHandler : IRequestHandler<GetTenantForEditQuery, Envelope<GetTenantForEditResponse>>
    {
        ...
        public async Task<Envelope<GetTenantForEditResponse>> Handle(GetTenantForEditQuery request, CancellationToken cancellationToken)
        {
            // Try to parse the tenant ID from the request.
            if (!Guid.TryParse(request.Id, out var tenantId))
                return Envelope<GetTenantForEditResponse>.Result.BadRequest(Resource.Invalid_tenant_Id);
            ...
            if (tenant == null)
                return Envelope<GetTenantForEditResponse>.Result.NotFound(Resource.Unable_to_load_tenant);

            // Convert the tenant entity to a response DTO.
            var tenantForEditResponse = GetTenantForEditResponse.MapFromEntity(tenant);

            // Return a successful response with the converted tenant DTO.
            return Envelope<GetTenantForEditResponse>.Result.Ok(tenantForEditResponse);
        }
