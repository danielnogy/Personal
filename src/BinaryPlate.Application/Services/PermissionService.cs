...
public class PermissionService(IApplicationDbContext dbContext, SignInManager<ApplicationUser> signInManager) : IPermissionService
{
    #region Public Methods

    public async Task<Envelope<GetPermissionsResponse>> GetPermissionsOnDemand(GetPermissionsQuery request)
    {
        // Get permissions from the database based on the query
        var permissionItems = await dbContext.ApplicationPermissions
        ...
        // Create a GetPermissionsResponse object containing the requested permissions
        ...
        return Envelope<GetPermissionsResponse>.Result.Ok(response);
    }

    public async Task<Envelope<GetPermissionsResponse>> GetAllPermissions()
    {
        ...
        // Retrieve all root permissions from the database (where the Parent is null) with the
        // necessary associations.
        var rootPermissions = await dbContext.ApplicationPermissions.Where(p => p.Parent == null).Include(p => p.Permissions).ToListAsync();
        ...
        // Create a new GetPermissionsResponse object and assign the retrieved permission items to it.
        var response = new GetPermissionsResponse
        {
    ...
    public async Task<List<PermissionItem>> GetUserPermissionsExceptExcluded(ApplicationUser user)
    {
        ...
        var excludedPermissions = await (from uc in dbContext.UserClaims
                                         join ap in dbContext.ApplicationPermissions
    ...
    private async Task<int> GetPermissionsMaxParentCount()
    {
        ...
        var maxParentCount = await dbContext.ApplicationPermissions.Select(p => p.ParentId).Distinct().CountAsync();
