...
public class StorageFactory : IStorageFactory
{
    ...
    public StorageFactory(IEnumerable<IFileStorageService> fileStorageServices)
    {
        ...
        // Initializes the _storageServicesByType field with a dictionary that maps StorageTypes to the corresponding IFileStorageService instances.
    ...
    public IFileStorageService CreateInstance(StorageTypes storageType)
    {
        ...
        if (!_storageServicesByType.ContainsKey(storageType))
            // If the storage type does not exist, throw an ArgumentException with a message indicating
            // the invalid storage type and the name of the parameter.
