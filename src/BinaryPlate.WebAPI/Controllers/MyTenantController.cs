...
public class MyTenantController : ApiController
{
    ...
    [ProducesResponseType(typeof(ApiSuccessResponse<GetMyTenantForEditResponse>), StatusCodes.Status200OK)]
    ...
    {
        var response = await Sender.Send(new DeleteMyTenantCommand { Id = id });
        return TryGetResult(response);
    }
