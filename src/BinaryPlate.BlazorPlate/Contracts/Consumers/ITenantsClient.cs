...
public interface ITenantsClient
{
    ...
    /// <returns>A <see cref="GetTenantForEditResponse"/>.</returns>
    Task<ApiResponseWrapper<GetTenantForEditResponse>> GetTenant(GetTenantForEditQuery request);

    /// <summary>
    /// Gets a list of tenants.
    /// </summary>
    /// <param name="request">The query parameters for retrieving the list of tenants.</param>
    /// <returns>A <see cref="GetTenantsResponse"/>.</returns>
    Task<ApiResponseWrapper<GetTenantsResponse>> GetTenants(GetTenantsQuery request);
