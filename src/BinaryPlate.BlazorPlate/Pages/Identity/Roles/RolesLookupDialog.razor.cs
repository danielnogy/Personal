...
public partial class RolesLookupDialog
{
    ...
    private GetRolesResponse GetRolesResponse { get; set; } = new();
    ...
    private void SelectAllVisibleRows(bool selected)
    {
        if (selected)
        {
            GetRolesResponse?.Roles.Items.ToList().ForEach(s => s.Checked = true);

            if (GetRolesResponse?.Roles.Items != null)
                SelectedUserRoles.AddRange(GetRolesResponse?.Roles.Items.Select(ur => new RoleItem
                {
                    Id = ur.Id,
                    Name = ur.Name,
                    Checked = ur.Checked,
        ...
        {
            GetRolesResponse?.Roles.Items.Where(r => r.Checked).ToList().ForEach(s => s.Checked = false);
            GetRolesResponse?.Roles.Items.ToList().ForEach(r => SelectedUserRoles.RemoveAll(sr => sr.Id == r.Id));
        }
    }
    ...
    private async Task<TableData<RoleItem>> ServerReload(TableState state)
    {
        ...
        if (responseWrapper.IsSuccessStatusCode)
        {
            ...
            if (SelectedUserRoles != null && SelectedUserRoles.Any())
                GetRolesResponse?.Roles.Items.Where(r => SelectedUserRoles.Select(sr => sr.Id).Contains(r.Id)).ToList().ForEach(s => s.Checked = true);
        }
        else
        {
        ...
        return new TableData<RoleItem> { TotalItems = GetRolesResponse?.Roles.TotalRows ?? 0, Items = GetRolesResponse?.Roles.Items ?? new List<RoleItem>() };
    }
