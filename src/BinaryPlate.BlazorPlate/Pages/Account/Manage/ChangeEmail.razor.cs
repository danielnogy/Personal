...
public partial class ChangeEmail
{
    ...
    private async Task Redirect(ChangeEmailResponse responseWrapper)
    {
        ...
        {
            var emailConfirmationUrlEncoded = responseWrapper.DisplayConfirmAccountLink
                ? $"account/manage/emailChangeConfirmation/{responseWrapper.DisplayConfirmAccountLink}/{WebUtility.UrlEncode(responseWrapper.EmailConfirmationUrl)}"
                : $"account/manage/emailChangeConfirmation/{responseWrapper.DisplayConfirmAccountLink}";
          
            NavigationManager.NavigateTo(emailConfirmationUrlEncoded);
        }
        else
        {
